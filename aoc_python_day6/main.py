...
def move(current, direction, map):
    ...
    new_pos = (current[0] + DIRECTIONS[direction][0], current[1] + DIRECTIONS[direction][1])
...
while True:
    try:
        ...
        size = 10
        subset_of_map = [line[current[1]-size:current[1]+size+1] for line in map[current[0]-size:current[0]+size+1]]
        print("\n".join([''.join(line) for line in subset_of_map]))
        print("="*20)
        # time.sleep(0.001)
    except IndexError:
        map[current[0]][current[1]] = 'X'
